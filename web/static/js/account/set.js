    ...
    eventBind:function () {
        $(".wrap_account_set .save").click(function () {
            ...
            if (nickname.length < 1){
                common_ops.tip("请输入符合规范的姓名js",nickname_target);
                return false;
            }
            if (mobile.length < 1){
                common_ops.tip("请输入符合规范的手机js",mobile_target);
                return false;
            }
            if (email.length< 1){
                common_ops.tip("请输入符合规范的邮箱js",email_target);
                return false;
            }
            if (login_name.length< 1){
                common_ops.tip("请输入符合规范的用户名js",login_name_target);
                return false;
            }
            if (login_pwd.length< 6){
                common_ops.tip("请输入符合规范的密码js",login_pwd_target);
                return false;
            }
